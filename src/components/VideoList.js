...
class VideoList extends Component {
  ...
  showStatus({ complete, timemark, outputPath, err }) {
    if (complete) {
      return (
        <button onClick={() => this.props.onFolderOpen(outputPath)} className="btn roundButtons">
          Open Folder
        </button>
      );
    } else if (err) {
  ...
  renderVideos() {
    return _.map(this.props.videos, video => {
      ...
      return (
        <li className="collection-item avatar not-draggable" key={path}>
          ...
          <div className="secondary-content" style={styles.secondaryContent}>
            <select
              ...
              style={{ borderRadius: '10px', outline: '0' }}
              value={format}
              onChange={e => this.props.onFormatChange(video, e.target.value)}
            >
              {VIDEO_FORMATS.map(outFormat => (
                <option style={{ borderRadius: '10px' }} key={outFormat.value} value={outFormat.value}>{outFormat.option}</option>
              ))}
            </select>
